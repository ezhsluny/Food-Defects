...
def analyze_histogram_needs(img_array):
    ...
    return needs_brightness, needs_contrast, mean_brightness, std_brightness
...
def advanced_preprocessing(img):
    ...
    # Коррекция контраста 
    if needs_contrast:
        # Используем растяжение гистограммы на основе перцентилей
        p2 = np.percentile(img_array_processed, 2)
        p98 = np.percentile(img_array_processed, 98)
        ...
        if p98 > p2:  
                ...
                r_contrast = (r_array - p2) * (255.0 / (p98 - p2))
                g_contrast = (g_array - p2) * (255.0 / (p98 - p2))
                b_contrast = (b_array - p2) * (255.0 / (p98 - p2))
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            ...
            if fps_res > 0:
                cv2.putText(img, f"FPS: {fps_res:.2f}", (10, 30), 
                          cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                cv2.putText(processed_img, f"FPS: {fps_res:.2f}", (10, 30), 
                          cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
