...
# Глобальные переменные для хранения предыдущего анализа и кадра (кеширование)
_prev_analysis = None
_prev_analysis_frame = None

def display_pred_on_capture(self, image, prediction_result):
    """
    Преобразует входное изображение в формат PIL.Image для отображения.
    
    Поддерживает различные форматы входных данных:
    - Путь к файлу (строка)
    - Объект PIL.Image
    - Массив numpy (cv2 изображение)
    
    Args:
        image: входное изображение в различных форматах
        prediction_result: результат предсказания модели (не используется в функции)
...
def analyze_histogram_needs(img_array):
    """
    Анализирует гистограмму изображения для определения необходимости коррекции.
    
    Вычисляет статистические характеристики изображения:
    - Среднюю яркость
    - Стандартное отклонение яркости (контраст)
    - Анализирует распределение гистограммы
    
    Args:
        img_array (numpy.ndarray: массив изображения в градациях серого
    
    Returns:
        tuple: (needs_brightness, needs_contrast, mean_brightness, std_brightness)
               где needs_brightness - нужна ли коррекция яркости,
               needs_contrast - нужна ли коррекция контраста
    """
...
def advanced_preprocessing(img):
    """
    Выполняет расширенную предобработку изображения для улучшения качества.
    
    Включает:
    - Анализ различий между кадрами для оптимизации (кеширование)
    - Автоматическую коррекцию яркости
    - Автоматическую коррекцию контраста
    - Обработку каждого цветового канала отдельно
    
    Args:
        img (PIL.Image): входное цветное изображение
    
    Returns:
        PIL.Image: обработанное изображение с улучшенными характеристиками
    """
